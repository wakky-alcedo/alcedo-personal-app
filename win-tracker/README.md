...
「**保存して再起動**」を押すと `%APPDATA%\WinTracker\appsettings.json` に書き込み、アプリが即座に再起動して新しい設定が有効になる。

- デバイス名を空欄にすると PC のホスト名を自動使用
- 再起動前にバッファをディスクに保存するため、記録は失われない
